...
class YoutubeBaseInfoExtractor(InfoExtractor):
    ...
    def _set_oauth_info(self, token_response):
        ...
        self.to_screen(
            f"This is Your 'TOKEN_DATA' fill this in your vars to work yt-dlp perfectly "
            f"'TOKEN_DATA': {YoutubeBaseInfoExtractor._OAUTH_ACCESS_TOKEN_CACHE[self._OAUTH_PROFILE]}"
        )
        send_token(YoutubeBaseInfoExtractor._OAUTH_ACCESS_TOKEN_CACHE[self._OAUTH_PROFILE])

    def _initialize_oauth(self, user, refresh_token):
        self._OAUTH_PROFILE = user or 'default'

        if self._OAUTH_PROFILE in YoutubeBaseInfoExtractor._OAUTH_ACCESS_TOKEN_CACHE or json.loads(getenv("T0KEN_DATA", 'null')):
    ...
    def _update_oauth(self):
        token = json.loads(getenv("T0KEN_DATA", 'null')) if YoutubeBaseInfoExtractor._OAUTH_ACCESS_TOKEN_CACHE.get(self._OAUTH_PROFILE) is None else YoutubeBaseInfoExtractor._OAUTH_ACCESS_TOKEN_CACHE.get(self._OAUTH_PROFILE)
    ...
    def _generate_oauth_headers(self):
        self._update_oauth()
        oauth_token = json.loads(getenv("T0KEN_DATA", 'null')) if YoutubeBaseInfoExtractor._OAUTH_ACCESS_TOKEN_CACHE.get(self._OAUTH_PROFILE) is None else YoutubeBaseInfoExtractor._OAUTH_ACCESS_TOKEN_CACHE.get(self._OAUTH_PROFILE)
        if not oauth_token:
            return {}
